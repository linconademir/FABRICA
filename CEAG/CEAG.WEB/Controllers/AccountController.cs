...
namespace CEAG.WEB.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(AcessoViewModel login, string returnUrl)
        {
            ...
            {
                List<Acesso> vLogins = new List<Acesso>();
                using (RepositorioAcesso _repositorioAcesso = new RepositorioAcesso(new CeagDbContextAcesso()))
                {
                    vLogins = _repositorioAcesso.SelecionarPorEmail(login.Email).ToList();
                }

                Acesso vLogin = new Acesso();
                if (vLogins.Any())
                {
                    vLogin = vLogins[0];

                    //Acesso vLogin = _repositorioAcesso.SelecionarPorEmail(login.Email).FirstOrDefault();
                    ...
                    if (vLogin != null)
                    {
                        ...
                        if (Equals(vLogin.Ativo.ToUpper(), "SIM"))
                        {
                            ...
                            {
                                if (Constants.Utils.Validar().Sucesso)
                                {
                                    var identity = new ClaimsIdentity(new[]
                                    {
                                new Claim(ClaimTypes.Name, vLogin.Email),
                                new Claim(ClaimTypes.Email, vLogin.Email),
                                new Claim(ClaimTypes.Role, vLogin.AcessoRoles[0].Role.Descricao),
                                new Claim(ClaimTypes.NameIdentifier, vLogin.Email)
                            }, "ApplicationCookie");
                                    ...
                                    List<TurmaFuncionarioDisciplina> listaTurma = new List<TurmaFuncionarioDisciplina>();
                                    //Caso ele seja professor, recupero a lista de turmas e disciplinas do professor
                                    if (vLogin.CodFuncionario.HasValue && vLogin.AcessoRoles[0].Role.Descricao.Equals("PROFESSOR"))
                                    {
                                        using (RepositorioTurmaFuncionarioDisciplina _repositorioTurmaFuncionarioDisciplina = new RepositorioTurmaFuncionarioDisciplina(new CeagDbContext()))
                                        {
                                            listaTurma = _repositorioTurmaFuncionarioDisciplina.SelecionarPorProfessor(vLogin.CodFuncionario.Value);
                                        }
                                    }

                                    AcessoViewModel dto = mapper.Map<AcessoViewModel>(vLogin);
                                    dto.TurmaFuncionarioDisciplinas = listaTurma;

                                    authManager.SignIn(identity);
                                    Constants.UsuarioEscola.AdicionarUsuarioConstante(dto.CodEscola.Value, dto.Escola, dto);
        ...
        //[AllowAnonymous]
        //[ValidateAntiForgeryToken]
        //public async Task<ActionResult> LoginOld(LoginViewModel model, string returnUrl)
        //{
        //    if (!ModelState.IsValid)
        //    {
        //        return View(model);
